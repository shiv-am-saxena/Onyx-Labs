...
export const MenuItem = ({ setActive, active, item, children }: { setActive: (item: string) => void; active: string | null; item: string; children?: React.ReactNode }) => {
	return (
		<div onMouseEnter={() => setActive(item)} className="relative backdrop-blur-md">
					...
					{active === item && children && (
						<div className="absolute top-[calc(100%_+_1.2rem)] left-1/2 transform -translate-x-1/2 pt-4">
							<motion.div
								...
								className="bg-[#18181b] rounded-2xl overflow-hidden border border-black/[0.2] dark:border-white/[0.2] shadow-xl">
								<motion.div
...
export const Menu = ({ setActive, children }: { setActive: (item: string | null) => void; children: React.ReactNode }) => {
	return (
		<nav
			...
			className="relative rounded-full border border-transparent backdrop-blur-md dark:border-white/[0.2] shadow-input flex justify-center space-x-4 px-8 py-6 ">
			{children}
		</nav>
	);
};
